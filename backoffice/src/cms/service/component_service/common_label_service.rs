...
use crate::cms::form::component_form::common_label_form::{CommonFormValidated, CommonLabelForm};
...
#[derive(Debug, Error)]
pub enum CommonLabelServiceError {
    #[error("db error")]
    DbError,
...
impl CommonLabelService {
    ...
    pub fn create_component(
        &self,
        query: &CreateQuery,
        common_label_form: &CommonLabelForm,
    ) -> Result<UpdateFetchQuery, Report<CommonLabelServiceError>> {
        let create_component_model = CreateComponentModel::<Vec<u8>> {
            page_id: query.page_id as i64,
            kind_uuid: self.cms_component_info.kind_uuid.clone(),
            raw_data: vec![],
            label: common_label_form.label.clone(),
        };

        let returning_id = self
            .cms_repository
            .create_component(create_component_model)
            .change_context(CommonLabelServiceError::DbError)?;
    ...
    ) -> Result<FetchComponentModel, Report<CommonLabelServiceError>> {
        self.cms_repository
            .fetch_component(query.id as i64)
            .change_context(CommonLabelServiceError::DbError)?
            .ok_or_else(|| {
                Report::new(CommonLabelServiceError::NotFound)
    ...
    pub fn update_component(
        &self,
        query: &UpdateFetchQuery,
        form: &CommonFormValidated,
    ) -> Result<(), Report<CommonLabelServiceError>> {
        self.cms_repository
...
impl FromContext for CommonLabelService {
